...
Technical self-audit date: 2026-08-07. Source of truth: `INTIMO_MVP_COMPLIANCE_AND_PRODUCT_REQUIREMENTS.md`. Status means repository implementation, not legal compliance. Database migrations are staged only and have not been applied. All policy text is a technical draft requiring counsel.
...
| CR-PRIV-002 private media | PARTIAL | `media_objects`, entitlement route, short signed downloads | Migration un-applied; upload completion/scanning and explicit declaration UI incomplete |
...
| CR-SAFETY-001 participant declarations | PARTIAL | declaration table and upload enforcement | Client currently supplies a default declaration; require explicit per-upload confirmation |
...
| CR-MSG-001/002 messaging authorization | PARTIAL | server-derived sender, participant and block checks | Delivery/read persistence and abuse controls need runtime integration tests |
| CR-MSG-003 attachments | FAIL | none | Private attachment storage, malware scanning, authorization and retention are not implemented |
| CR-SEC-001/002 authorization/security | PARTIAL | verified Auth0 session, allowlists, server actor, rate/size/type limits | Direct legacy client storage/writes and external config validation remain |
...
Current technical readiness: **52/100 — NO-GO** for public explicit-adult launch. The free MVP constraint itself is enforced; the remaining blockers are safety, privacy operations, deployment configuration, testing and legal approval.
