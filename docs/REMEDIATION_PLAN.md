...
- The smallest safe persistence target is PostgreSQL/Supabase as the durable database, with versioned `supabase/migrations/*.sql` as the canonical schema history and authenticated Next.js APIs as the server data-access boundary. Prisma and `db/schema.sql` remain frozen legacy references until deployed-schema parity is recorded; `supabase/migration.sql` is a legacy bootstrap snapshot and must not be applied to production. In-memory/localStorage stores remain demo fixtures only and must not back production behavior.
- Discovery and ranking are deterministic, documented, and testable. No AI features are in scope.

## Phases

1. **Identity and secret containment (in progress):** credential fallbacks are removed; supported Auth0 SDK sessions are canonical; production fails closed for missing credentials or a non-Intimo tenant; onboarding/settings are server-guarded; verification has an explicit invalid-session state; account deletion and media presigning are session-owned. Remaining release work is owner correction of the production Auth0 application variables, reviewed deployment, and controlled live acceptance testing.
