 ...
 */
export async function validateAuth0BearerToken(bearerHeader: string | undefined): Promise<ApiSecurityResult> {
  const jwtCheck = await JwtValidatorService.validateAuth0Token(bearerHeader);
