...
const PROFILE_MEDIA_REFERENCE = /^\/api\/media\/([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
const SELF_SERVICE_PROFILE_FIELDS = new Set([
  "display_name", "avatar_url", "cover_photo_url", "country", "city", "location",
  "languages", "headline", "bio", "interests", "lifestyle_tags", "hobbies",
...
export async function POST(req: NextRequest) {
  ...
  const sexualOrientation = typeof body.sexualOrientation === "string" ? body.sexualOrientation : "";
  if (!ALLOWED_GENDERS.has(body.gender) || (sexualOrientation && !ALLOWED_ORIENTATIONS.has(sexualOrientation))) return NextResponse.json({ error: "Invalid demographic selection" }, { status: 400 });
  const sensitiveDataProvided = Boolean(sexualOrientation);
  if (sensitiveDataProvided && body.explicitSensitiveDataConsent !== true) return NextResponse.json({ error: "Explicit consent is required to process sexual orientation" }, { status: 400 });
...
export async function PUT(req: NextRequest) {
  ...
  const mapped = profileToDbRow(identity.sub, identity.email || "", profile);
  const update = Object.fromEntries(
    Object.entries(mapped).filter(([field]) => SELF_SERVICE_PROFILE_FIELDS.has(field)),
  ) as Record<string, unknown>;
  if (Object.keys(update).length === 0) return NextResponse.json({ error: "No supported profile changes were supplied" }, { status: 400 });
  ...
  const { data: owner, error: ownerError } = await supabase
    ...
    .maybeSingle();
  if (ownerError || !owner) return NextResponse.json({ error: "Profile not found" }, { status: 404 });

  const verifyOwnedMedia = async (field: "avatar_url" | "cover_photo_url", category: "PROFILE_AVATAR" | "PROFILE_COVER") => {
    if (!(field in update)) return true;
    if (update[field] === "" || update[field] === null) {
      update[field] = null;
      return true;
    }
    if (typeof update[field] !== "string") return false;
    const match = update[field].match(PROFILE_MEDIA_REFERENCE);
    ...
    const { data: media, error: mediaError } = await supabase
      ...
      .maybeSingle();
    return !mediaError && Boolean(media);
  };

  if (!(await verifyOwnedMedia("avatar_url", "PROFILE_AVATAR"))) {
    return NextResponse.json({ error: "Profile photo reference is invalid or is not owned by this account" }, { status: 400 });
  }
  if (!(await verifyOwnedMedia("cover_photo_url", "PROFILE_COVER"))) {
    return NextResponse.json({ error: "Cover photo reference is invalid or is not owned by this account" }, { status: 400 });
  }
  ...
  const { data: before, error: beforeError } = await supabase
    .from("profiles")
    .select(["id", ...changedFields].join(","))
    .eq("auth_id", identity.sub)
    .maybeSingle();
  if (beforeError || !before) return NextResponse.json({ error: "Profile not found" }, { status: 404 });
  ...
  const audited = await appendDurableAudit(supabase, { actorProfileId: data.id, actorAuth0Sub: identity.sub, action: "PROFILE_UPDATE", resourceType: "PROFILE", resourceId: String(data.id), outcome: "SUCCESS", metadata: { changedFields } });
  if (!audited) {
    const previous = before as unknown as Record<string, unknown>;
    const rollback = Object.fromEntries(changedFields.map((field) => [field, previous[field]]));
    rollback.updated_at = new Date().toISOString();
    await supabase.from("profiles").update(rollback).eq("id", data.id).eq("auth_id", identity.sub);
    return NextResponse.json({ error: "Profile changes were not retained because audit recording failed" }, { status: 503 });
  }
  return noStore(NextResponse.json({ profile: data }));
}
