...
export async function GET(req: NextRequest) {
  ...
  const { data: albums, error } = await db.from("media_albums").select("id,title,description,category,tags,visibility,moderation_status,cover_media_id,published_at,created_at,updated_at").eq("owner_id", auth.actor.profileId).neq("moderation_status", "REMOVED").order("updated_at", { ascending: false }).limit(100);
...
export async function POST(req: NextRequest) {
  ...
  const db = getServerSupabase(); if (!db) return NextResponse.json({ error: "Albums unavailable" }, { status: 503 });
  const { data, error } = await db.from("media_albums").insert({ owner_id: auth.actor.profileId, title, description: description || null, category: category || null, tags, visibility, moderation_status: "PENDING_REVIEW" }).select("id,title,description,category,tags,visibility,moderation_status,created_at").single();
