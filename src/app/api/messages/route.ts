...
export async function POST(req: NextRequest) {
  ...
  const { data: recipient } = await supabase.from("profiles").select("id,display_name,message_permission,allow_direct_messages").eq("id", receiverId).maybeSingle();
  ...
  const { data: sender } = await supabase.from("profiles").select("display_name,avatar_url").eq("id", actor.actor.profileId).single();
