...
export default function SearchPage() {
  ...
  const [reportTarget, setReportTarget] = React.useState<{ id: string; ownerId: string; ownerName: string; type: "PHOTO" | "VIDEO" | "POST" } | null>(null);

  React.useEffect(() => {
    if (submitted.length < 2) return;
    const controller = new AbortController(); setLoading(true); setError("");
    const endpoint = user ? "/api/search" : "/api/public/search";
    fetch(`${endpoint}?q=${encodeURIComponent(submitted)}`, { credentials: "same-origin", cache: "no-store", signal: controller.signal })
  ...
  return <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-300">Community search</p><h1 className="mt-2 font-serif text-3xl font-semibold text-white">Search Intimo</h1><p className="mt-2 max-w-2xl text-sm text-slate-400">{user ? "Find visible members, approved media, active dating ads, and moderated posts. Block and privacy settings are enforced before results are returned." : "Search public member profiles, approved public media, albums, and active dating listings. Sign in for member-only results and interactions."}</p>
    ...
    <ReportModal isOpen={Boolean(reportTarget)} onClose={() => setReportTarget(null)} targetUsername={reportTarget?.ownerName} targetProfileId={reportTarget?.ownerId} contentType={reportTarget?.type} contentId={reportTarget?.id} />
